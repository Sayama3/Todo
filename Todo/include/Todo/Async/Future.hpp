...
namespace Todo
{
    ...
    std::future_status Future<T>::wait_for(const std::chrono::duration<Rep, Period>& duration) const
    {
    ...
    std::future_status Future<T>::wait_until(const std::chrono::time_point<Clock, Duration>& time_point) const
    {
        if (!m_Result) { throw std::future_error(std::future_errc::no_state); }
